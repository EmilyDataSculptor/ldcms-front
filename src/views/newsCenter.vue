        ...
        <div class="con1400">
            ...
            <template>  
              <div class="block">  
                <el-carousel height="500px">  
                  <el-carousel-item v-for="theNews in responseData.slice(0, 6)" :key="theNews.id" class="carousel-item-with-bg">  
                    <div  @click="$router.push(`/newsCenterDetails?id=${theNews.id}`)">
                      <div class="carousel-item-content">  
                      <img :src="theNews.picture" class="carousel-image" />  
                      <div class="carousel-text">  
                        <p class="right-aligned-theNewsText">{{ theNews.mainTitle }}</p>  
                        <p class="right-aligned-theNewsText2">{{ theNews.content.slice(0, 90) + (theNews.content.length > 90 ? '......' : '') }}</p>  
                        <p class="right-aligned-theNewsText3">{{ theNews.creationDate.slice(0,10) }}</p>
          ...
          <div class="newslistp_bot container">
            <ul class="row">
              ...
              data-animate="zoomIn" data-duration="1.5s" data-delay="0.1s"  v-for="theNewsList in responseDataList" :key="theNewsList.id">
                <div @click="$router.push(`/newsCenterDetails?id=${theNewsList.id}`)" title="复制文档到网站后台，怎么样才能清除格式">
                  <div class="newslistp_botimg">
                    <img :src="theNewsList.picture" alt="复制文档到网站后台，怎么样才能清除格式">
                  </div>
                  <div class="newslistp_botdes flexrow">
                    <h3>{{theNewsList.mainTitle}}</h3>
                    <p>阅读全文 ></p>
                  </div>
                </div>
              </li>
...
export default {
  ...
  methods:{
    ...
    selectListByNewsCenter() {  
      axios.get(this.baseUrl+'newsCenter/selectListByNewsCenter',{
        params: {  
    ...
    .then(response => {  
          // 处理成功响应，例如显示成功消息 
        this.responseDataList = response.data.data;
        this.total = Number(response.data.msg.split("_")[1]); 
        console.log("this.responseDataList",this.responseDataList)
    ...
    handleCurrentChange(val) {  
     ...
     updateArticle(product){
      this.$router.push(`/newsCenterDetails?id=${product.id}`)
      this.$router.go(0); 
    },
