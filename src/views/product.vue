        ...
        <div class="prolist_item flexrow">
              ...
              <div class="prolist_lbot">
                <ul>
                    ...
                    <div class="erpronav">
                      <ul>
                            <li>
                          <a class="flexrow" href="/product" :style="{backgroundColor:clickChangeColour}" @click="toggleAllBlue">
                            
                            <span>全部</span>
                            <img src="../ldcms2024/images/jiantblue.png" alt="">
                          </a></li>

                          <li v-for="product in responseColour" :key="product.id">
                            <a class="flexrow" href="#"  @click.prevent="selectListByColourTap(product)" :style="{backgroundColor:product.color}">
                    ...
                    <div class="erpronav   ">
                      <ul>
                        <li>
                          <a class="flexrow" href="/product" :style="{backgroundColor:clickChangeType}" @click="toggleAllBlue">
          ...
          <div class="prolist_r animate" data-animate="fadeInRight" data-duration="1.5s" data-delay="0.1s">
            <div class="prolist_list container">
              <ul class="row">  
              <li v-for="product in products" :key="product.id" class="col-lg-4 col-md-4 col-sm-6 col-xs-12">  
                <div @click="$router.push(`/productSubpageByDetails?id=${product.id}`)"  :title="product.mainTitle">  
                  ...
                  <div class="prolist_item_text">  
                    ...
                    <p>{{ product.content.slice(0, 15) + (product.content.length > 15 ? '...' : '') }}</p>  
                  </div>  
                </div>  
              </li>  
            </ul> 
...
<script>
  ...
  export default {
    ...
    // 回到第一页的方法  
    goToFirstPage() {  
        this.currentPage = 1; // 设置当前页码为第一页  
        // 可能还需要调用加载方法，具体取决于您的需求  
        // 例如，如果您想在回到第一页时重新加载数据：  
        if (this.methodCalledByColour === 0) {  
            this.selectListByProduct();  
        } else if (this.methodCalledByColour === 1) {  
            this.selectListByColour();  
    ...
    selectByDistinctType() {  
      axios.get(this.baseUrl+'product/selectByDistinctType',{ 
        ...
        // console.log('后端返回this.responseType', this.responseType);
      }).catch(error => {  
        // 处理错误  
       console.error(error);  
    });
    ...
    mounted(){
      jQuery(document).ready(function () {
                jQuery('.pronav1').click(function () {
                    jQuery(this).next('.erpronav').slideToggle();
            })
        }); 
