...
<div>
 ...
 <div class="top hidden-xs">
    ...
    <section class="neinav">
      <div class="con1400">
        <div class="neinav_item flexrow">
          <ul class="flexrow">
                        <li class="">
              <a href="/solution" title="行业方案">行业方案</a>
            </li>
                        <li class=" active ">
              <a href="/solution#fashzr" title="社会责任">社会责任</a>
            </li>
                      </ul>
          <div class="casetop_r">
            <p>
              ...
              <a href="/solution">解决方案</a> 
                            >
              <a href="/solutionBySocialResponsibility">社会责任</a> 
                          </p>
          </div>
        </div>
      </div>
    ...
    <section class="neimain">
      <div class="con1400">
        <div class="newsdetail flexrow">
          <div class="newsde_l animate"
          data-animate="fadeInLeft" data-duration="1.5s" data-delay="0.1s">
            <h2>{{ socialResponsibility.mainTitle }}</h2>
            <div class="newsde_ltime flexrow">
              <div class="newsdel_left">
                <p>
                                    <a href="/solution#fashzr">#社会责任</a>
                                    <span>·</span><i>{{ dateTimeString }}</i></p>
              </div>

              <div class="newsdel_right">
                <!-- <a class="qrcode_ewm" href="javascript:;">
                  <img src="../ldcms2024/images/wxIcon.png" alt>
                </a> -->
                ...
                <div class="qrcode_ewmimg" id="qrcode"></div>
                <el-button type="text" @click="open">
                    <a class="a-weibo" href="javascript:;">
                  <img src="../ldcms2024/images/qqIcon.png" alt>
                </a></el-button>

              </div>
            </div>
            <div class="newsde_lcontent">
              <p> {{ socialResponsibility.content }} </p>
            </div>
               <div class="newsde_lbot flexrow">
                <el-row>
              <el-button round @click="upPages()">上一页</el-button>
              </el-row>
                            <div class="newdelb_m">
                <a href="/solution#fashzr">返回列表</a>
              </div>
              
              <el-row>
              <el-button round @click="downPages()">下一页</el-button>
              </el-row>
          
              </div>
          </div>
          <div class="newsde_r animate"
          data-animate="fadeInRight" data-duration="1.5s" data-delay="0.1s">
            <ul>
                            <li v-for="product in randomFourProducts" :key="product.id">
                <div @click="updateArticle(product)"  title="携手西安交通大学苏州附属中学 助力打造“纳米科学创新人才培养项目实验班”_copy_copy">
                  <div class="newsde_rimg">
                    <img :src="product.picture" alt="携手西安交通大学苏州附属中学 助力打造“纳米科学创新人才培养项目实验班”_copy_copy">
                  </div>
                  <div class="newsde_rdesc">
                    <h3>{{product.mainTitle}}</h3>
                    <p>{{product.creationDate.slice(0, 10)}}</p>
                  </div>
                </div>
              </li>
...
<script>
  ...
  export default {
    ...
    data() {  
        ...
        //selectAllBySocialResponsibility返回的数据
        responseData:[],
    ...
    created() {
        this.selectAllBySocialResponsibility();
    },

    methods: { 

        open() {
        this.$alert('<div style="text-align: center;"><img src="../wxpicture.jpg" alt="咨询微信客服" style="width: 300px;"></div>', '请扫码添加联系管理员', {
          dangerouslyUseHTMLString: true
        });
      },

      selectAllBySocialResponsibility() {  
        axios.get(this.baseUrl+'socialResponsibility/selectAllBySocialResponsibility',{ 
          params: {  
        ...
        .then(response => {  
            ...
            const position = this.findPositionById(this.id,this.responseData);
            this.position=position;
            console.log("position",position);
            console.log("this.id",this.id);

            const nextId = this.findPositionByNextId(this.id,this.responseData);
            this.nextId= nextId;
            console.log("nextId",nextId);

            const previousId = this.findPositionByPreviousId(this.id,this.responseData);
            this.previousId= previousId;
            console.log("previousId",previousId);
            
            this.selectSocialResponsibilityById();
            })
        .catch(error => {  
        // 处理错误  
        console.error(error);  
    ...
    findPositionById(theId, list) {  
        for (let i = 0; i < list.length; i++) {  
            if (list[i].id === Number(theId)) { // 假设每个社会责任信息对象都有一个 'id' 属性 
    ...
    findPositionByNextId(theId, list) {  
        for (let i = 0; i < list.length; i++) {  
            if (list[i].id === Number(theId)) { 
              if(i+1 === list.length){
                return list[0].id;
              }else{
                return list[i+1].id; 
    ...
    findPositionByPreviousId(theId, list) {  
        for (let i = 0; i < list.length; i++) {  
            if (list[i].id === Number(theId)) { 
              if(i === 0){
    ...
    updateArticle(product){
      this.$router.push(`/solutionBySocialResponsibility?id=${product.id}`)
      this.$router.go(0); 
    },

    downPages(){
      let  downPagePosition=this.nextId;
      this.$router.push(`/solutionBySocialResponsibility?id=${downPagePosition}`)
      this.$router.go(0); 
    },

    upPages(){
      let  upPagePosition=this.previousId;
      this.$router.push(`/solutionBySocialResponsibility?id=${upPagePosition}`)
      this.$router.go(0); 
    },

    selectSocialResponsibilityById() {  
          const id = this.$route.query.id; 
          this.id=id;
          axios.get(this.baseUrl+`socialResponsibility/selectSocialResponsibilityById?id=${this.id}`,{ 
    })
          .then(response => {  
              ...
              this.socialResponsibility = response.data.data[0];
              console.log("this.socialResponsibility",this.socialResponsibility)
              this.dateTimeString =this.socialResponsibility.creationDate;    
              var dateString = this.dateTimeString.split(' ')[0];  
    ...
    computed: {  
        ...
        randomFourProducts() {  
      // 首先，确保responseData不是空的  
      if (!this.responseData.length) return [];  
  
      // 复制一份responseData，避免直接修改原始数据  
      let products = [...this.responseData];  
